...
final class TopicManager: ObservableObject {
    ...
    func moveTopic(from source: IndexSet, to destination: Int) {
        ...
        // Find where the previously-active topic ended up
        let newPinned: Int?
        if let p = pinned {
    ...
    /// Returns the next Date when `index` becomes the active topic.
    /// For the currently-active topic, returns the start of the current half-day slot.
    func scheduleDate(forIndex index: Int) -> Date? {
        ...
        // How many half-day slots ahead is this index?
    ...
    /// Adjust offset so that `activeIndex` returns `desiredIndex` (if non-nil and valid).
    private func restoreActive(_ desiredIndex: Int?) {
        ...
        // We need: (halfDaySlot + offset) % count == desired
